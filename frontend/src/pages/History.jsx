      ...
      <div className="cyber-panel overflow-hidden">
        {loading ? (
          <div className="p-8 text-center text-gray-400 flex flex-col items-center gap-2">
            <svg className="animate-spin h-6 w-6 text-green-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
            Loading history...
          </div>
        ) : error ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#1a1a1a] border-b border-gray-700 text-gray-400 text-xs uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-border">
                {filteredLogs.map((log) => (
                  <tr key={log.id + log.operation} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4 text-sm font-mono text-gray-400 max-w-[200px] truncate" title={log.message}>
                      {log.message}
                    </td>
